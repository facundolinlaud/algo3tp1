...
def preplot(runs):
	...
	for cardinal, stats in runs[algorithm].items():
		...
		cota_ys.append(cardinal * math.pow(2, cardinal) * O_de_1)
...
def plot(lines):
	...
	########## scatter ##########
	plt.scatter([], [], c='rebeccapurple', label='Brute Force')
	for xe, ye in zip(line['xs'], line['yss']):
		plt.scatter([xe] * len(ye), ye, c="rebeccapurple", alpha=0.3)

	# pdb.set_trace()
	plt.scatter([], [], c='magenta', label='Brute Force (Promedio por cada n)')
	for xs, ys in zip(line['xs'], line['ys']):
		plt.scatter([xs], [ys], c="magenta", alpha=1)


	########## error bar con promedios de prodin ########
	# plt.errorbar(line['xs'], line['ys'], yerr=line['yerror'], label='Back Tracking', color='gold', 
	...
	cota = lines['cota']
	plt.errorbar(cota['xs'], cota['ys'], label=r'Cota $n*2^n$', color='C3', linewidth=1, xerr=None, fmt='o-.',
	...
	# 	label='Aprox. lineal Back Tracking', alpha=0.8)
	...
	# 	color='firebrick', label=r'Aprox. lineal cota $n*2^n$', alpha=0.8)
	##############
